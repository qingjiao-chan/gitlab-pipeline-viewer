...
> **安全提示**：访问令牌以**明文**保存在本机 IDEA 配置目录
> （`options/gitlab-pipeline-viewer.xml`，仅限当前用户可读）。令牌只发送给你在设置中填写的
> GitLab 地址，本插件不上传任何数据到第三方。请勿在共享 / 公共机器上使用，并妥善保管该文件；
> 如令牌泄露，请立即到 GitLab 后台撤销并重新生成。
